...
class RCTVideo: UIView, RCTVideoPlayerViewControllerDelegate, RCTPlayerObserverHandler {
    ...
    private func setupNotificationObservers() {
        let notificationObservers = [
            ...
            (UIApplication.didEnterBackgroundNotification, #selector(applicationDidEnterBackground)),
            (UIApplication.willEnterForegroundNotification, #selector(applicationWillEnterForeground)),
            (UIApplication.protectedDataWillBecomeUnavailableNotification, #selector(screenWillLock)),
            (UIApplication.protectedDataDidBecomeAvailableNotification, #selector(screenDidUnlock)),
            (AVAudioSession.routeChangeNotification, #selector(audioRouteChanged)),
            (AVAudioSession.interruptionNotification, #selector(audioSessionInterruption)),
            (NSNotification.Name("RCTVideo.nextTrack"), #selector(handleNextTrack)),
            (NSNotification.Name("RCTVideo.previousTrack"), #selector(handlePreviousTrack))
        ]
    ...
    func initPictureinPicture() {
            ...
            if _pip == nil {
                _pip = RCTPictureInPicture(
                    ...
                    { [weak self] in self?.onRestoreUserInterfaceForPictureInPictureStop?([:]) }
                )
            }
    ...
    @objc func setQueue(_ queue: NSArray!) {
        ...
        // Always activate queue mode when a queue is set
        _videoQueue = queue
        _isQueueMode = true
        
        if _player?.currentItem != nil && _player?.rate ?? 0 > 0 {
            _currentQueueIndex = -1  // Will be incremented to 0 when current ends
        } else {
    ...
    @objc func handlePreviousTrack() {
        if _isQueueMode {
            ...
            onPreviousTrack?(["target": reactTag as Any, "nativeHandled": false])
        }
    }
    ...
    private func playNextInQueue() {
        ...
        if _currentQueueIndex >= 0 && _currentQueueIndex < _videoQueue.count {
            ...
            // Ensure background playback continues during queue transition
            let wasPlaying = !_paused && _isPlaying
            let isBackgroundTransition = UIApplication.shared.applicationState != .active
            
            setSrc(nextSource)
            
            // Force resume playback for background/lock screen queue transitions
            if wasPlaying && (isBackgroundTransition || _playInBackground) {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                    ...
                    // Ensure Now Playing is updated for lock screen controls
                    if self._showNotificationControls, let player = self._player {
                        NowPlayingInfoCenterManager.shared.registerPlayer(player: player, videoView: self)
    ...
    private func handleBackgroundTransition() {
        ...
        // Queue mode background handling - maintain continuous playback
        if _playInBackground && _isQueueMode && !isExternalPlaybackActive && !isPictureInPictureActive() {
            // Ensure audio session is properly configured for background queue playback
            configureAudioSession()
            
            // Maintain playback during background transition for queue mode
            _player?.play()
            _player?.rate = _rate
            
            // Register with Now Playing for background controls
            if _showNotificationControls, let player = _player {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
                    guard let self = self else { return }
                    NowPlayingInfoCenterManager.shared.registerPlayer(player: player, videoView: self)
                    
                    // Ensure metadata is maintained in background
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        NowPlayingInfoCenterManager.shared.updateNowPlayingInfo()
                    }
                }
    ...
    private func handleForegroundTransition() {
        // Queue mode foreground handling - ensure continuous playback
        if _isQueueMode && _playInBackground {
            configureAudioSession()
            
            // Ensure queue playback continues when returning to foreground
            if !_paused {
                _player?.play()
                _player?.rate = _rate
            }
            ...
            if _showNotificationControls, let player = _player {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                    ...
                    // Ensure metadata is updated when returning to foreground
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        NowPlayingInfoCenterManager.shared.updateNowPlayingInfo()
                    }
                }
        ...
        // Ensure metadata is available after foreground transition
        ensureMetadataForNowPlaying()
    }
        ...
        if _wasPlayingBeforeInterruption {
            ...
            if _showNotificationControls, let player = _player {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                    ...
                    // Force metadata update after interruption recovery
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        NowPlayingInfoCenterManager.shared.updateNowPlayingInfo()
                    }
                }
    ...
    private func updatePlayerViewControllerFrame(_ playerViewController: RCTVideoPlayerViewController) {
    ...
    private func preparePHAssetPlayerItem(uri: String, source: VideoSource) async throws -> AVPlayerItem {
    ...
    private func prepareRegularPlayerItem(source: VideoSource) async throws -> AVPlayerItem {
    ...
    func setupPlayer(playerItem: AVPlayerItem) async throws {
        ...
        // Store current playback state for queue transitions
        let wasPlayingBeforeTransition = !_paused && _isPlaying
        let isBackgroundMode = UIApplication.shared.applicationState != .active
        let shouldMaintainPlayback = _isQueueMode && _playInBackground && (wasPlayingBeforeTransition || isBackgroundMode)
        
        // Only pause if not maintaining playback for queue transitions
        ...
        // Restore playback immediately for queue transitions in background
        if shouldMaintainPlayback {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                guard let self = self else { return }
    ...
    private func createNewPlayer(with playerItem: AVPlayerItem) {
        ...
        // Register with Now Playing after a brief delay to ensure metadata is processed
    ...
    private func updateExistingPlayer(with playerItem: AVPlayerItem) {
        ...
        // Force an immediate update for existing players
    ...
    func playerItemPrepareText(source: VideoSource, asset: AVAsset!, assetOptions: NSDictionary?, uri: String) async -> AVPlayerItem {
        ...
        // Always propagate metadata for Now Playing Info Center
        return await playerItemPropagateMetadata(playerItem)
    }
    
    func playerItemPropagateMetadata(_ playerItem: AVPlayerItem!) async -> AVPlayerItem {
        var mapping: [AVMetadataIdentifier: Any] = [:]
        
        // Set custom metadata if available
    ...
    func sendProgressUpdate(didEnd: Bool = false) {
        ...
        guard let video = _player?.currentItem, video.status == .readyToPlay else { return }
    ...
    @objc func setSeek(_ time: NSNumber, _ tolerance: NSNumber) {
        guard let item = _player?.currentItem, let player = _player, item.status == .readyToPlay else {
    ...
    // MARK: - Additional Property Setters
    ...
    @objc func setControls(_ controls: Bool) {
        guard _controls != controls || (_playerLayer == nil && _playerViewController == nil) else { return }
    ...
    @objc func setShowNotificationControls(_ showNotificationControls: Bool) {
        ...
        if showNotificationControls {
            // Delay registration to ensure player item and metadata are ready
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                guard let self = self else { return }
                NowPlayingInfoCenterManager.shared.registerPlayer(player: player, videoView: self)
    ...
    private func addPlayerViewControllerToView(_ playerViewController: RCTVideoPlayerViewController) {
    ...
    @objc func setFullscreen(_ fullscreen: Bool) {
        ...
        if fullscreen && !_fullscreenPlayerPresented && _player != nil && !alreadyFullscreenPresented {
            presentFullscreen()
        } else if !fullscreen && _fullscreenPlayerPresented, let playerViewController = _playerViewController {
    ...
    private func presentFullscreen() {
        ...
        guard let viewController = findPresentingViewController(),
              let playerViewController = _playerViewController else { return }
    ...
    private func dismissFullscreen(_ playerViewController: RCTVideoPlayerViewController) {
        videoPlayerViewControllerWillDismiss(playerViewController: playerViewController)
        _presentingViewController?.dismiss(animated: true) { [weak self] in
            self?.videoPlayerViewControllerDidDismiss(playerViewController: playerViewController)
        }
        setControls(_controls)
    ...
    private func findPresentingViewController() -> UIViewController? {
        ...
        if viewController == nil, let keyWindow = RCTVideoUtils.getCurrentWindow() {
    ...
    @objc func setPreventsDisplaySleepDuringVideoPlayback(_ preventsDisplaySleepDuringVideoPlayback: Bool) {
    ...
    @objc func setEnterPictureInPictureOnLeave(_ enterPictureInPictureOnLeave: Bool) {
        #if os(iOS)
    ...
    @objc func setRestoreUserInterfaceForPIPStopCompletionHandler(_ restore: Bool) {
        #if os(iOS)
            if _pip != nil {
    ...
    @objc func setPreferredForwardBufferDuration(_ preferredForwardBufferDuration: Float) {
    ...
    @objc func setDisableAudioSessionManagement(_ disableAudioSessionManagement: Bool) {
    ...
    @objc func setFilter(_ filterName: String!) {
        ...
        Task {
            let composition = await RCTVideoUtils.generateVideoComposition(asset: asset, filter: filter)
    ...
    @objc func getCurrentPlaybackTime(_ resolve: @escaping RCTPromiseResolveBlock, _ reject: @escaping RCTPromiseRejectBlock) {
    ...
    @objc func save(_ options: NSDictionary!, _ resolve: @escaping RCTPromiseResolveBlock, _ reject: @escaping RCTPromiseRejectBlock) {
        RCTVideoSave.save(options: options, resolve: resolve, reject: reject, playerItem: _playerItem)
    }
    ...
    // MARK: - Workarounds
    @objc func setOnClick(_: Any) {} // Workaround for #3418
    ...
    private func updatePlayerViewControllerLayout(_ playerViewController: RCTVideoPlayerViewController) {
        playerViewController.view.frame = bounds
        playerViewController.view.setNeedsLayout()
        playerViewController.view.layoutIfNeeded()
    ...
    // MARK: - Helper Methods
    private func ensureMetadataForNowPlaying() {
        // Ensure Now Playing gets updated with current metadata when available
        if _showNotificationControls, let player = _player, player.currentItem?.status == .readyToPlay {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                guard let self = self else { return }
                NowPlayingInfoCenterManager.shared.registerPlayer(player: player, videoView: self)
    ...
    private func processReadyToPlay(playerItem: AVPlayerItem, source: VideoSource) async {
        if _pendingSeek {
        ...
        // Ensure Now Playing is updated when player item is ready and metadata is available
        if _showNotificationControls, let player = _player {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                guard let self = self else { return }
                NowPlayingInfoCenterManager.shared.registerPlayer(player: player, videoView: self)
                
                // Force metadata update after registration
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    NowPlayingInfoCenterManager.shared.updateNowPlayingInfo()
                }
            }
    ...
    private func dispatchVideoLoadEvent(playerItem: AVPlayerItem, source: VideoSource) async {
        ...
        let (width, height, orientation) = await getVideoProperties(playerItem: playerItem)
        ...
        onVideoLoad?([
            ...
            "currentTime": NSNumber(value: Float(CMTimeGetSeconds(playerItem.currentTime()))),
    ...
    private func getVideoProperties(playerItem: AVPlayerItem) async -> (Float, Float, String) {
        var width: Float = 0
        var height: Float = 0
        ...
        let orientation = width > height ? "landscape" : width == height ? "square" : "portrait"
    ...
    func handlePlaybackFailed() {
        ...
        guard let playerItem = _playerItem, let error = playerItem.error else { return }
    ...
    private func handleRepeat(notification: NSNotification!) {
        ...
        let seekTime = _source?.cropStart != nil ?
    ...
    // MARK: - RCTPlayerObserverHandler Implementation
    ...
    func handlePlayerItemStatusChange(playerItem _: AVPlayerItem, change _: NSKeyValueObservedChange<AVPlayerItem.Status>) {
        ...
        case .readyToPlay:
            handleReadyToPlay()
            // Ensure metadata is available when player item becomes ready
            ensureMetadataForNowPlaying()
        case .failed:
    ...
    func handleTimeControlStatusChange(player: AVPlayer, change: NSKeyValueObservedChange<AVPlayer.TimeControlStatus>) {
        guard player.timeControlStatus != change.oldValue,
              [.paused, .playing].contains(player.timeControlStatus) else { return }
    ...
    func handleVolumeChange(player: AVPlayer, change: NSKeyValueObservedChange<Float>) {
        guard onVolumeChange != nil, player.volume != change.oldValue else { return }
    ...
    func handleExternalPlaybackActiveChange(player: AVPlayer, change _: NSKeyValueObservedChange<Bool>) {
        #if !os(visionOS)
            ...
            onVideoExternalPlaybackChange?([
                "isExternalPlaybackActive": NSNumber(value: player.isExternalPlaybackActive),
                "target": reactTag as Any
            ])
        #endif
    }
    ...
    func handleViewControllerOverlayViewFrameChange(overlayView _: UIView, change: NSKeyValueObservedChange<CGRect>) {
        guard let oldRect = change.oldValue,
              ...
              let bounds = RCTVideoUtils.getCurrentWindow()?.bounds else { return }
    ...
    func handleTracksChange(playerItem _: AVPlayerItem, change _: NSKeyValueObservedChange<[AVPlayerItemTrack]>) {
        ...
        if onTextTracks != nil {
            Task {
                let textTracks = await RCTVideoUtils.getTextTrackInfo(_player)
                self.onTextTracks?([
                    "textTracks": extractJsonWithIndex(from: source.textTracks) ?? textTracks.compactMap(\.json)
                ])
            }
        }
    ...
    func handleLegibleOutput(strings: [NSAttributedString]) {
        guard onTextTrackDataChanged != nil, let subtitles = strings.first else { return }
    ...
    @objc func handlePlayerItemDidReachEnd(notification: NSNotification!) {
        ...
        // Handle repeat mode first (takes precedence over queue)
        if _repeat {
        ...
        // Default: pause player (no queue, no repeat)
        pausePlayer()
    }
